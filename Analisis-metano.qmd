...
df <- metano %>% arrange(desc(Emisiones))  # Ordenar de mayor a menor emisión

# Obtener los 5 países con más y menos emisiones
top_5 <- df %>% head(5)
bottom_5 <- df %>% tail(5)

# Gráficamo los 5 países con más emisiones
g1 <- ggplot(top_5, aes(x = reorder(Country, -Emisiones), y = Emisiones)) +
  geom_bar(stat = "identity", fill = "red") +
  coord_flip() +
  labs(x = "País", y = "Emisiones de metano", title = "Top 5 países con más emisiones") +
  theme_minimal()
...
```{r}
# Cargamos el dataframe
df <- metano %>% arrange(Emisiones)  # Ordenar de menor a mayor emisión

# Obtener los 5 países con más y menos emisiones
top_5 <- df %>% tail(5)  # Ahora toma los últimos 5 valores correctamente
bottom_5 <- df %>% head(5)  # Selecciona los primeros 5 con menos emisiones

# Gráficamos
g2 <- ggplot(bottom_5, aes(x = reorder(Country, Emisiones), y = Emisiones)) +
  geom_bar(stat = "identity", fill = "green") +
  coord_flip() +
  labs(x = "País", y = "Emisiones de metano", title = "Top 5 países con menos emisiones") +
  theme_minimal()
...
# Filtrar solo los países mostradois anteriormente
selected_countries <- c("Ecuador", "Dinamarca", "Cuba", "Bangladesh", "Albania")
filtered_data <- metano %>% filter(Country %in% selected_countries)

filtered_data$Emisiones <- as.numeric(filtered_data$Emisiones)
filtered_data <- filtered_data %>% filter(!is.na(Emisiones) & Emisiones >= 0)

# Crear el gráfico sin escala logarítmica
g_final <- ggplot(filtered_data, aes(x = reorder(Country, Emisiones), y = Emisiones)) +
  geom_col(fill = "green") +
  coord_flip() +
  labs(x = "País", y = "Emisiones de metano", title = "Emisiones de metano en países seleccionados") +
  theme_minimal()
...
Lo cual indica que su significancia estadística es menor en comparación con la variable agriculture.
...
Las regiones que más emisiones de metano producen a nivel global son Asia-Pacific, North America y Russia-Caspian. Lo cual es un indicador de la escala de producción e industrialización de sus sectores agrícolas, ganaderos y energéticos de los países que conforman dichas regiones, Principalmente China, Rusia y Estados Unidos. 

El resto de las regiones muestra un balance relativo en la emisión de metano por tipo de industrias.
Los países con menor producción de metano se localizan en América Latina, y Europa. (Ecuador, Dinamarca, Albania).
